...
// Define constants
const WOX_TOKEN_ADDRESS = '0xb9A219631Aed55eBC3D998f17C3840B7eC39C0cc'.toLowerCase();
...
// Function to get logs in chunks to avoid RPC timeout
async function getLogsInChunks(filter, chunkSize = 2000) {
  // If fromBlock and toBlock are specified, use them
  if (filter.fromBlock !== undefined && filter.toBlock !== 'latest') {
    const fromBlock = typeof filter.fromBlock === 'number' ? filter.fromBlock : parseInt(filter.fromBlock);
    const toBlock = typeof filter.toBlock === 'number' ? filter.toBlock : parseInt(filter.toBlock);
    
    const logs = [];
    
    for (let i = fromBlock; i <= toBlock; i += chunkSize) {
      const chunkToBlock = Math.min(i + chunkSize - 1, toBlock);
      ...
      try {
        const chunkLogs = await web3.eth.getPastLogs(chunkFilter);
        logs.push(...chunkLogs);
      } catch (error) {
        console.error(`Error getting logs for blocks ${i} to ${chunkToBlock}:`, error.message);
      }
    }
  ...
  const currentBlock = await web3.eth.getBlockNumber();
  const fromBlock = typeof filter.fromBlock === 'number' ? filter.fromBlock : parseInt(filter.fromBlock || '0');
  
  const logs = [];
  
  for (let i = fromBlock; i <= currentBlock; i += chunkSize) {
    const chunkToBlock = Math.min(i + chunkSize - 1, currentBlock);
...
// Function to find token creation block using binary search
async function getTokenCreationBlock(tokenAddress, maxBlock = null) {
  if (!maxBlock) {
    maxBlock = await web3.eth.getBlockNumber();
  }
  ...
  console.log(`Searching for creation block of token ${tokenAddress} between blocks ${minBlock} and ${maxBlock}`);
  
  while (minBlock <= maxBlock) {
    const midBlock = Math.floor((minBlock + maxBlock) / 2);
    
    // Check if token exists at midBlock
    try {
      const code = await web3.eth.getCode(tokenAddress, midBlock);
      
      if (code.length > 2) { // "0x" is returned for non-existent contracts
        // Token exists at this block, search lower
        result = midBlock;
        maxBlock = midBlock - 1;
      } else {
        // Token doesn't exist yet, search higher
        minBlock = midBlock + 1;
      }
    } catch (error) {
      console.error(`Error checking code at block ${midBlock}:`, error.message);
      minBlock = midBlock + 1;
    }
  }
...
// Function to get token price from your exchange (placeholder)
async function getTokenPriceFromExchange(tokenAddress, quoteTokenAddress, blockNumber = 'latest') {
  // This is a placeholder function - in a real implementation, you would:
  // 1. Query your exchange contract for the current price
  // 2. Or check liquidity pools for the token pair
  
  // For now, we'll return a fixed price of 0.5 WOX for all tokens
  // Replace this with your actual price discovery logic
  return 0.5;
}

// Function to calculate token price in terms of WOX
async function calculateTokenPriceInWOX(tokenAddress, blockNumber = 'latest') {
  // If the token is WOX itself, return 1 (1 WOX = 1 WOX)
  if (tokenAddress.toLowerCase() === WOX_TOKEN_ADDRESS) {
    return 1.0;
  }
  
  try {
    // Get price from exchange
    const price = await getTokenPriceFromExchange(tokenAddress, WOX_TOKEN_ADDRESS, blockNumber);
    ...
    return 0.5; // Default to 0.5 WOX on error
  }
}

// Function to calculate price history from transfer events
async function calculatePriceHistoryFromTransfers(logs, tokenAddress) {
  // If there are no logs, return a single data point with the current price
  if (!logs || logs.length === 0) {
    const currentTime = Math.floor(Date.now() / 1000);
    const currentPrice = await calculateTokenPriceInWOX(tokenAddress);
    
    return [{
      price: currentPrice,
      time: currentTime * 1000,
      timeFormatted: moment.unix(currentTime).toISOString()
    }];
  }
  
  // Sort logs by block number and timestamp
  logs.sort((a, b) => {
  ...
  // For price history, we'll sample at regular intervals
  // Get the earliest and latest block/timestamp
  const earliestLog = logs[0];
  const latestLog = logs[logs.length - 1];
  
  const startTime = earliestLog.timestamp;
  const endTime = latestLog.timestamp;
  
  // Calculate time range and determine appropriate sampling interval
  const timeRange = endTime - startTime;
  
  // Aim for ~100 data points across the time range
  const targetDataPoints = 100;
  const samplingInterval = Math.max(3600, Math.floor(timeRange / targetDataPoints)); // At least 1 hour
  
  // Create sampling points
  const samplingPoints = [];
  for (let t = startTime; t <= endTime; t += samplingInterval) {
    samplingPoints.push(t);
  }
  
  // Add the exact end time if it's not already included
  if (samplingPoints[samplingPoints.length - 1] !== endTime) {
    samplingPoints.push(endTime);
  }
  
  // Create a mapping of timestamp to nearest block number
  const timestampToBlockMap = {};
  for (const log of logs) {
    const timestamp = log.timestamp;
    if (!timestampToBlockMap[timestamp] || log.blockNumber > timestampToBlockMap[timestamp]) {
      timestampToBlockMap[timestamp] = log.blockNumber;
    }
  }
  
  // For each sampling point, find the nearest log and calculate price
  const priceData = [];
  
  for (const samplingTime of samplingPoints) {
    // Find the nearest log timestamp
    let nearestTimestamp = samplingTime;
    let nearestDiff = Infinity;
    
    for (const logTimestamp in timestampToBlockMap) {
      const diff = Math.abs(parseInt(logTimestamp) - samplingTime);
      if (diff < nearestDiff) {
        nearestDiff = diff;
        nearestTimestamp = parseInt(logTimestamp);
      }
    }
    
    // Get the block number for this timestamp
    const blockNumber = timestampToBlockMap[nearestTimestamp];
    
    // Calculate price at this block
    const price = await calculateTokenPriceInWOX(tokenAddress, blockNumber);
    
    priceData.push({
      price: price,
      time: samplingTime * 1000,
      timeFormatted: moment.unix(samplingTime).toISOString()
    });
  }
...
app.get('/token-graph/:tokenAddress', async (req, res) => {
  ...
  try {
    ...
    // Calculate price history from transfers
    const priceData = await calculatePriceHistoryFromTransfers(logsWithTimestamps, normalizedTokenAddress);
    
    // Return the price data array
    ...
    const results = await Promise.all(
      validTokenAddresses.map(async (tokenAddress) => {
        try {
          ...
          // Calculate price history from transfers
          const priceData = await calculatePriceHistoryFromTransfers(logsWithTimestamps, tokenAddress);
